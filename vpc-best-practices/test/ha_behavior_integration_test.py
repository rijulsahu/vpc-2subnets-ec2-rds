...
def create_test_tfvars():
    ...
    with open('../ha-test.tfvars', 'w') as f:
...
def deploy_vpc_infrastructure():
    ...
    stdout, stderr, code = run_command('tofu plan -var-file=ha-test.tfvars -out=ha-test.tfplan', cwd='..')
    if code != 0:
    ...
    stdout, stderr, code = run_command('tofu apply -auto-approve ha-test.tfplan', cwd='..')
    if code != 0:
...
def create_test_instances(outputs):
    ...
    instance_ids.append(bastion_instance_id)
    print_pass(f"Launched bastion instance: {bastion_instance_id}")
    
    # Wait for instances
    print_info("Waiting for instances to be running...")
    time.sleep(30)
    wait_command = f"aws ec2 wait instance-running --instance-ids {' '.join(instance_ids)}"
    run_command(wait_command)
    
    # Get instance details
    describe_command = f"aws ec2 describe-instances --instance-ids {' '.join(instance_ids)} --output json"
    ...
    for instance in instances:
        if instance['InstanceId'] == bastion_instance_id:
    ...
    print_info(f"Bastion: {bastion_instance_id} (Public IP: {bastion_ip})")
    for az, details in private_ips.items():
        print_info(f"Instance in {az}: {details['instance_id']} (Private IP: {details['private_ip']})")
...
def test_multi_az_deployment(outputs):
    ...
    if len(azs) >= 2:
        print_pass(f"Subnets distributed across {len(azs)} AZs: {', '.join(azs)}")
    else:
...
def test_baseline_connectivity(resources):
    ...
    print_info("Waiting for instances to fully initialize...")
    time.sleep(60)
    
    # Test connectivity from each instance
    all_connected = True
    for az, details in resources['private_ips'].items():
        print_info(f"Testing connectivity from {az}...")
        
        test_command = f"""ssh -o StrictHostKeyChecking=no \
            -i {resources['key_file']} ec2-user@{resources['bastion_ip']} \
        ...
        stdout, stderr, code = run_command(test_command, check=False)
...
def test_ha_failover(resources, failure_info):
    ...
    other_azs = [az for az in resources['private_ips'].keys() if az != affected_az]
    
    # Test that affected AZ cannot reach internet
    print_info(f"Verifying {affected_az} is affected...")
    affected_details = resources['private_ips'][affected_az]
    ...
    test_command = f"""ssh -o StrictHostKeyChecking=no \
        ...
        'timeout 15 curl -s -o /dev/null -w %{{http_code}} --connect-timeout 10 http://amazon.com'" """
    ...
    for az in other_azs:
        ...
        details = resources['private_ips'][az]
...
def cleanup_test_resources(resources, outputs):
    ...
    # Clean up test files
    if os.path.exists('../ha-test.tfvars'):
        os.remove('../ha-test.tfvars')
    if os.path.exists('../ha-test.tfplan'):
        os.remove('../ha-test.tfplan')
